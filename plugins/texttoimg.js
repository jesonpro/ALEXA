...
Neotro.addCommand({ pattern: ' ?(.*)', fromMe: tn,dontAddCommandList: true}, (async (message, match) => {
        ...
        var ttinullimage = await axios.get(`${config.HTTPS}${config.HLOCK}${config.SUP_HEROKU}${config.ENCRYPTION}${config.TOXIC_T}?teks=${encodeURIComponent(match[1])}`, { responseType: 'arraybuffer' })
